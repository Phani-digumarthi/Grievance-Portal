...
import { 
  ...
  LogOut, History, Image as ImageIcon, Volume2
} from "lucide-react";
import dynamic from "next/dynamic";
import { useRouter } from "next/navigation"; // Added router for Logout
...
export default function AdminDashboard() {
  ...
  const handleLogout = () => {
    // Clear admin session (if you added one)
  ...
  const count = (key, val) => grievances.filter(g => g[key] === val).length;
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-20 relative">
      ...
      <div className="bg-white p-4 border-b border-slate-200 flex justify-between items-center sticky top-0 z-20 shadow-sm">
        ...
        <div className="flex items-center gap-3">
            ...
            <button onClick={handleLogout} className="flex items-center gap-2 px-4 py-2 text-sm font-bold text-red-600 bg-red-50 hover:bg-red-100 rounded-lg transition-colors border border-red-100">
                                ...
                                {selectedGrievance.imageUrl && (
                                    <div className="space-y-1">
                                        ...
                                        <div className="relative rounded-xl overflow-hidden border border-slate-200 shadow-sm">
                                            <img 
                                                src={`http://localhost:5000/${selectedGrievance.imageUrl}`} 
                                                alt="Grievance Evidence" 
                                                className="w-full h-auto object-cover max-h-60 hover:scale-105 transition-transform duration-500"
                                            />
                                        </div>
                                    </div>
                                )}
